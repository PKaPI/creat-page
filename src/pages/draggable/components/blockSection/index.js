...
export default class BlockSection extends Component {
  ...
  getReactBlocks = ()=>{
    ...
    fetch('https://raw.githubusercontent.com/dtux-kangaroo/ko-config/master/roo-react-blocks.json').then((res) => {
      return res.json();
    }).then((res) => {
        let { type, blocks } = res
        const sorterList = {}
  ...
  renderContent = () => {
    ...
    for (let key in sorterList) {
      let childrenList =searchKey ? sorterList[key].filter(item=>item['title'].indexOf(searchKey)>-1):sorterList[key];
      
      childrenList.length&&template.push(
        <div key={key}>
          <div className="title_count">{key}<span>{`(${sorterList[key].length})`}</span></div>
          <div className="cards">
            {
              childrenList.map((item, index) => (
              <div className={classNames({ card:true })} onClick={()=>this.onSelectBlock(item)} key={index}>
                <div className="box" >
  ...
  render() {
    const { imageSrc,activeKey, loading, visibleFilter, filterList, filterKey } = this.state;
    const {className} = this.props;
    return (
      <div className={classNames('page-blocks',className)} >
        <div className="block_title">区块展示</div>
          <Tabs activeKey={activeKey} onChange={this.onChangeActiveKey} tabBarExtraContent={
        ...
        <div className="pageControl">
          <div><Icon type="smile" className="filter_menu" onClick={this.showDrawer} /></div>
        </div>
        <Viewer
